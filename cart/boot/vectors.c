// SH7021 vector table.
//
// Taken from Kasami's loopy-homebrew-template, used under the zlib licence --
// see LICENSE-loopy-template.txt. ALTERED from the original: ITU1 drives the
// millisecond clock (platform/lp_clock.c), and the SCI vectors are unused.
...
/* Set nonzero, nonmasked priority in IPRA-IPRB to enable */
	...
	// ITU0 belongs to the BIOS, for BGM.
	bios_isrAutoUpdateBgm, doNothing, doNothing, RESERVED,
	// ITU1: compare match A drives the millisecond clock and pad sampling.
	LP_ClockISR, doNothing, doNothing, RESERVED,
