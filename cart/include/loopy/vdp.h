// Casio Loopy VDP registers and video memory map.
//
// Taken from the loopy-homebrew-template by Kasami, used under the zlib
// licence -- see LICENSE-loopy-template.txt and NOTICE.md.
//
// ALTERED from the original: the VDP object is declared
// volatile. See the note above its declaration at the end of this file.
...
// Declared volatile: these are memory-mapped registers, and without it the
// compiler is free to merge two adjacent 16-bit register stores into a single
// 32-bit store. The VDP decodes register writes by access width, so a merged
// 32-bit write to e.g. BLEND+LAYER_CTRL is not the same operation at all.
